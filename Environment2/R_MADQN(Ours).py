...
#file_name = 'test'
result_dir = '/content/drive/MyDrive/modi_env/improve_R_MADQN/epsilon/3state_2reccurent/SN'+str(args.amount_SN)+'/'+str(file_name)
...
class ReplayBuffer:
    ...
    def put(self,state, state_1_list, state_2_list,return_state_1_before,return_state_2_before, action_1, action_2, reward, next_state, next_state_1_list, next_state_2_list,return_state_1_after,return_state_2_after, done,res_next_actions_list_1, res_next_actions_list_2):
        self.buffer.append([state, state_1_list, state_2_list,return_state_1_before,return_state_2_before, action_1, action_2, reward, next_state, next_state_1_list, next_state_2_list,return_state_1_after,return_state_2_after, done,res_next_actions_list_1, res_next_actions_list_2])
        #self.buffer.put(state, state_1_list, state_2_list,return_state_1_before,return_state_2_before, action_1, action_2, reward, next_state, next_state_1_list, next_state_2_list,return_state_1_after,return_state_2_after, done,res_next_actions_list_1, res_next_actions_list_2)#reward×0.01しているん何故？？
    
    def sample(self):
        sample = random.sample(self.buffer, args.batch_size)
        #state, state_1_list, state_2_list,return_state_1_before,return_state_2_before, action_1, action_2, reward, next_state, next_state_1_list, next_state_2_list,return_state_1_after,return_state_2_after, done,res_next_actions_list_1, res_next_actions_list_2
        states, state_1_lists, state_2_lists, return_state_1_before_s, return_state_2_before_s,action_1_s, action_2_s, rewards, next_states,next_state_1_lists, next_state_2_lists, return_state_1_after_s, return_state_2_after_s, done,res_next_actions_list_1_s,res_next_actions_list_2_s = map(np.asarray, zip(*sample))
        ...
        return_state_1_before_s = np.array(return_state_1_before_s).reshape(args.batch_size, -1)
        state_1_lists = np.concatenate((state_1_lists,return_state_1_before_s),axis=1)

        state_2_lists = np.array(state_2_lists).reshape(args.batch_size, -1)
        return_state_2_before_s = np.array(return_state_2_before_s).reshape(args.batch_size, -1)
        state_2_lists = np.concatenate((state_2_lists,return_state_2_before_s),axis=1)

        next_state_1_lists = np.array(next_state_1_lists).reshape(args.batch_size, -1)
        return_state_1_after_s = np.array(return_state_1_after_s).reshape(args.batch_size, -1)
        next_state_1_lists = np.concatenate((next_state_1_lists,return_state_1_after_s),axis=1)

        next_state_2_lists = np.array(next_state_2_lists).reshape(args.batch_size, -1)
        return_state_2_after_s = np.array(return_state_2_after_s).reshape(args.batch_size, -1)
        next_state_2_lists = np.concatenate((next_state_2_lists,return_state_2_after_s),axis=1)
        ...
        return  states, state_1_lists, state_2_lists, return_state_1_before_s, return_state_2_before_s,action_1_s, action_2_s, rewards, next_states,next_state_1_lists, next_state_2_lists, return_state_1_after_s, return_state_2_after_s, done,res_next_actions_list_1_s,res_next_actions_list_2_s
    
    def max_sample(self):
        max_sample = self.buffer
        states, state_1_lists, state_2_lists, return_state_1_before_s, return_state_2_before_s,action_1_s, action_2_s, rewards, next_states,next_state_1_lists, next_state_2_lists, return_state_1_after_s, return_state_2_after_s, done,res_next_actions_list_1_s,res_next_actions_list_2_s = map(np.asarray, zip(*max_sample))
        state_1_lists = np.array(state_1_lists).reshape(self.size(), -1)
        return_state_1_before_s = np.array(return_state_1_before_s).reshape(self.size(), -1)
        state_1_lists = np.concatenate((state_1_lists,return_state_1_before_s),axis=1)

        state_2_lists = np.array(state_2_lists).reshape(self.size(), -1)
        return_state_2_before_s = np.array(return_state_2_before_s).reshape(self.size(), -1)
        state_2_lists = np.concatenate((state_2_lists,return_state_2_before_s),axis=1)

        next_state_1_lists = np.array(next_state_1_lists).reshape(self.size(), -1)
        return_state_1_after_s = np.array(return_state_1_after_s).reshape(self.size(), -1)
        next_state_1_lists = np.concatenate((next_state_1_lists,return_state_1_after_s),axis=1)

        next_state_2_lists = np.array(next_state_2_lists).reshape(self.size(), -1)
        return_state_2_after_s = np.array(return_state_2_after_s).reshape(self.size(), -1)
        next_state_2_lists = np.concatenate((next_state_2_lists,return_state_2_after_s),axis=1)
...
class ActionStateModel:
    ...
    def create_model(self):
        model = tf.keras.Sequential([
            InputLayer(((self.state_dim - 2)*3+self.state_dim*2,)),
            Dense(self.state_dim*2, activation='relu',name='Layer_0'),
    ...
    def hidden_1_outputs(self,return_state,state_1_list):
        return_state = np.reshape(return_state, [1, self.state_dim*2])
        state_1_list =np.reshape(state_1_list, [1, (self.state_dim-2)*3])
    ...
    def hidden_2_outputs(self,return_state,state_2_list):
        return_state = np.reshape(return_state, [1, self.state_dim*2])
        state_2_list =np.reshape(state_2_list, [1, (self.state_dim-2)*3])
    ...
    def get_action_1(self, return_state, state_list):
        return_state = np.reshape(return_state, [1, self.state_dim*2])
        res_actions_list = res_actions(state_list[2][1],state_list[2][2])
        state_list =np.reshape(state_list, [1, (self.state_dim-2)*3])
...
class Agent:
    ...
    def replay_1(self):
        for _ in range(1):
            states, state_1_lists, state_2_lists, return_state_1_before_s, return_state_2_before_s,action_1_s, action_2_s, rewards, next_states,next_state_1_lists, next_state_2_lists, return_state_1_after_s, return_state_2_after_s, done,res_next_actions_list_1_s,res_next_actions_list_2_s = self.buffer.sample()
            targets = self.target_model.predict_on_batch_1(state_1_lists)
            next_q_values = self.target_model.predict_on_batch_1(next_state_1_lists)
    ...
    def replay_2(self):
        for _ in range(1):
            ...
            next_q_values = self.target_model.predict_on_batch_2(next_state_2_lists)
    ...
    def evaluate_by_buffer(self):
        states, state_1_lists, state_2_lists, return_state_1_before_s, return_state_2_before_s,action_1_s, action_2_s, rewards, next_states,next_state_1_lists, next_state_2_lists, return_state_1_after_s, return_state_2_after_s, done,res_next_actions_list_1_s,res_next_actions_list_2_s = self.buffer.max_sample()
        targets_1 = self.target_model.predict_on_batch_1(state_1_lists)
        next_q_values_1 = self.target_model.predict_on_batch_1(next_state_1_lists)
        ...
        next_q_values_2 = self.target_model.predict_on_batch_2(next_state_2_lists)
    ...
    def train(self, max_episodes=10000):
        ...
        for ep in range(max_episodes):
            ...
            step_count=0
            state_1_list=np.array([state_1, state_1, state_1])
            next_state_1_list=state_1_list
            state_2_list=np.array([state_2, state_2, state_2])
            next_state_2_list=state_2_list
            return_state_1_before=[]
            return_state_2_before=[]
            for i in range(2*self.state_dim):
              return_state_1_before.append(0)
              return_state_2_before.append(0)
            #print(return_state_1_before)
            while not done:
                AoI_list=[int(state[1+2+2+i]) for i in range(amount_SN)]
                average_AoI = sum(AoI_list) / amount_SN
                action_1 = self.model.get_action_1(return_state_1_before,state_1_list)
                action_2 = self.model.get_action_2(return_state_2_before,state_2_list)
    ...
    def test_1(self, max_episodes=10):
        ...
        for ep in range(max_episodes):
            ...
            print('ep{} AoI/U={} / {} state_pos={} total_average_AoI={}'.format(ep, AoI_list, [state[0],state[1],state[2],state[3],state[4]], U_list, total_average_AoI/step_count))
